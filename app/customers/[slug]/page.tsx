...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    description: customer.story?.summary ?? customer.headline,
    ...
    <main className="min-h-screen overflow-hidden bg-white text-black">
      ...
      <section className="relative overflow-hidden bg-[#eef4ff] px-6 pb-12 pt-[112px] md:pt-[132px]">
        ...
        <div className="relative z-10 mx-auto max-w-[920px] text-center">
          ...
          <p className="mt-7 text-[15px] font-bold uppercase tracking-[0.14em] text-[#3559e9]">{customer.brand}</p>
          <h1 className="mx-auto mt-4 max-w-[820px] text-[32px] font-semibold leading-[1.14] tracking-[-0.04em] text-[#05070d] md:text-[46px]">
            {customer.headline}
          </h1>
          {story ? (
            ...
            <div className={`mt-6 grid gap-5 ${story.results.length >= 4 ? "sm:grid-cols-2 lg:grid-cols-4" : "sm:grid-cols-3"}`}>
              {story.results.map((r) => (
                <div key={r.label} className="rounded-2xl border border-slate-200 bg-white p-5 text-center">
                  <strong className="block text-[30px] font-bold tracking-[-0.03em] text-[#2465f6] md:text-[36px]">{r.value}</strong>
      ...
      {story ? (
          ...
          <div className="mx-auto flex max-w-[760px] flex-col gap-14">
            {story.sections.map((section) => (
              <div key={section.title}>
                <h2 className="text-[13px] font-semibold uppercase tracking-[0.12em] text-[#3559e9]">{section.title}</h2>
                <div className="mt-4 flex flex-col gap-4">
                  {section.body.map((p, i) => (
                    <p key={i} className="text-[18px] leading-[1.65] text-slate-700">
                      {p}
                    </p>
                  ))}
                </div>
                {section.quote ? (
                  <figure className="mt-7 border-l-[3px] border-[#3559e9] pl-6">
                    <blockquote className="text-[21px] font-medium leading-[1.45] tracking-[-0.01em] text-slate-900 md:text-[23px]">
                      “{section.quote.text}”
                    </blockquote>
                    <figcaption className="mt-3 text-[14px] text-slate-500">
                      <span className="font-semibold text-slate-700">{section.quote.name}</span> · {section.quote.role}
                    </figcaption>
                  </figure>
                ) : null}
              </div>
        ...
        <section className="px-6 py-10">
          <figure className="mx-auto max-w-[760px] text-center">
            ...
            <blockquote className="mt-4 text-[22px] font-medium leading-[1.5] text-slate-800 md:text-[26px]">{customer.quote}</blockquote>
          ...
          <div className="mx-auto max-w-[760px]">
            ...
            <div className="mt-5 overflow-hidden rounded-2xl border border-slate-200">
              ...
              {story.numbers.map((n, i) => (
                <div
                  key={n.label}
                  className={`grid grid-cols-[1.4fr_1fr_1fr] items-center px-5 py-4 text-[15px] ${i > 0 ? "border-t border-slate-100" : ""}`}
                >
              ...
              {related.map((c) => (
                ...
                >
                  <div className="relative aspect-[16/11] overflow-hidden bg-slate-100">
                    ...
                    <span className="absolute left-4 top-4 inline-flex rounded-full bg-white/90 px-3 py-1 text-[12px] font-semibold text-[#3559e9] backdrop-blur">{c.category}</span>
                    <span className="absolute bottom-4 left-4 text-[16px] font-bold text-white drop-shadow-sm">{c.brand}</span>
                  </div>
                  <div className="flex flex-1 flex-col p-6">
                    <p className="text-[15px] font-semibold leading-[1.4] text-slate-900">{c.headline}</p>
